...
// sharing its meta.name with a hand-authored commands/*.md file of the same base name
// silently shadows that command — the command's --help, argument-hint, and any other body
// logic become unreachable dead code, exactly what happened to commands/audit-docs.md
// against workflows/audit-docs.js (renamed to audit-docs-engine.js in the fix for #191).
// This test fails if that collision is ever reintroduced, for audit-docs or any future
// command/workflow pair in this plugin.
...
function workflowMetaNames() {
  ...
  return readdirSync(dir)
    ...
    .map(f => {
      ...
      const match = source.slice(metaStart).match(/name:\s*['"]([^'"]+)['"]/);
      assert.ok(match, `${f}'s meta object has no name: '...'/"..." anywhere after 'export const meta'`);
